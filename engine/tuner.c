...
DEB_0(MAT_DUO(mat[i], mat[i+1], p->pawn_weak_onopen_penalty[0], p->pawn_weak_onopen_penalty[1], i); i+=2;)
...
#pragma omp critical
	  ...
	  if(ix!=-1) {
		  ...
		  nj.res=res;
		  res2=populate_njac (&b, &nj, b.pers, mx, pcount);
...
{
			ll=counter++;
			if(ll>=max) ix=-1; else state[ll]=nj;
}
