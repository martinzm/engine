...
int f;
	...
	for(f=0;f<64;f++) {
		...
		case ER_PIECE : 
			break;
		case BISHOP :	key^=RP[64*5+f];
			break;
		case KNIGHT :	key^=RP[64*3+f];
			break;
		case PAWN  :	key^=RP[64*1+f];
			break;
		case QUEEN :	key^=RP[64*9+f];
			break;
		case KING :		key^=RP[64*11+f];
			break;
	...
	switch(b->castle[WHITE]) {
		case 0:
			break;
		case KINGSIDE: key^=RC[0];
			break;
		case QUEENSIDE: key^=RC[1];
			break;
		case BOTHSIDES: key=key^RC[1]^RC[0];
			break;
	}
	switch(b->castle[BLACK]) {
		case 0:
			break;
		case KINGSIDE: key^=RC[2];
			break;
		case QUEENSIDE: key^=RC[3];
			break;
		case BOTHSIDES: key=key^RC[3]^RC[2];
			break;
	}
	if(b->side==WHITE) key^=RT[0];
	if(b->ep!=-1) {
		if(((attack.ep_mask[b->ep]) & (b->maps[PAWN]) & (b->colormaps[b->side]))!=0) {
			key^=RE[getFile(b->ep)];
		}
	}
	*k=key;
...
char buf[512];
	...
	sprintf(buf, "Book move, Choosen record: %i, p: %i out of %i", p2,p,r);
	LOGGER_1("INFO: ",buf, "\n");
...
char buf[512];
	{
			...
			sprintf(buf, "Book file: %s loaded. %i records.", filename, orecords);
			LOGGER_1("INFO: ",buf, "\n");
	}
	return 1;
}
